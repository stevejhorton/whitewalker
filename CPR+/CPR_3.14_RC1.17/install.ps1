...
$DocsDeployDir       = "C:\ProgramData\WhiteWalker\docs"
...
function Ensure-Directories {
    ...
    $directories = @($MainScriptDeployDir, $CapPortalDeployDir, $ConfigDeployDir, $LogDir, $DiagDir, $DocsDeployDir)
...
function Test-Installation {
    ...
    # Check docs dir deployed (content validated by presence + file count)
    Write-InstallLog "Checking docs directory..." "DEBUG"
    if (Test-Path $DocsDeployDir) {
        $docFiles = (Get-ChildItem -Path $DocsDeployDir -File -ErrorAction SilentlyContinue).Count
        Write-InstallLog "  OK: docs/ ($docFiles file(s))" "DEBUG"
    } else {
        Write-InstallLog "  MISSING: $DocsDeployDir" "WARN"  # WARN not ERROR - docs optional at deploy time
    }
...
Write-InstallLog "========================================" "INFO"
Write-InstallLog "WhiteWalker Installer RC1.14-merged" "INFO"
Write-InstallLog "========================================" "INFO"
...
$docsOK    = Copy-DirectoryContents -SourceDir "$ScriptRoot\docs"  -DestDir $DocsDeployDir       -Label "docs"
$testsOK   = Copy-DirectoryContents -SourceDir "$ScriptRoot\tests" -DestDir $MainScriptDeployDir -Label "tests"
