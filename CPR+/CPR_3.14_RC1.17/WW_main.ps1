    ...
    if (-not $ciscoPath) { 
        ...
        if (-not (Test-Path $vpn_cmd)) { 
            ...
            try {
                ...
                # Start Cisco browser killer BEFORE initial_sleep so any Cisco browser
                # already open at WW entry is killed immediately. Job is non-blocking
                # (Start-Job) and runs for 120s to cover the full WW execution window.
