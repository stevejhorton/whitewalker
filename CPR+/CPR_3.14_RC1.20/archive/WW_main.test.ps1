...
<#
  ...
  Version: 3.14.0_RC1.8
  Author: steve.horton@optum.com (with AI assist)
  Date: 16-Mar-2026

  Major changes in 3.14.0_RC1.8:
    - PERF: WLANi03 fast path - skips gateway preflight, goes direct to enroll.cisco.com
    ...
    - WLANi03 is always on-prem corp wireless - no need to probe gateway first
...
$BlackholeScriptPath    = "C:\ProgramData\WhiteWalker\blackhole.ps1"
...
function Invoke-BlackholeAction {
    param([string]$Tag)
    if (-not $BlackholeEnabled) { return }
    ...
    if ($onPremTags -contains $Tag) {
        ...
        # Any other state - if flag exists, remove blackhole entries immediately
        # CRITICAL: -rm must fire before VPN is ever attempted or headends are unreachable
        if (Test-Path $BlackholeFlagFile) {
            if (-not (Test-Path $BlackholeScriptPath)) {
                Write-Log "Blackhole enabled but script not found: $BlackholeScriptPath" "WARN"
                return
            }
            Write-Log "Blackhole: non-on-prem state ($Tag) with flag present - firing -rm in background" "INFO"
            if (-not $WhatIf) {
                Start-Process "powershell.exe" `
                ...
                Write-Log "Blackhole: -rm fired, flag removed" "INFO"
            } else {
                Write-Log "WHATIF: Would run blackhole.ps1 -rm and remove $BlackholeFlagFile" "INFO"
            }
        }
    }
}
...
function Test-DC { 
    ...
    # ER6: Enhanced DC test - don't trust ping alone, check network context
    try {
        # First check: Can we ping the DC?
        ...
        # ER6: Second check - is this via active WiFi with corporate SSID?
        # This prevents false positives when stale VPN routes exist on captive portals
        try {
            ...
            if ($wifiAdapter) {
                # On WiFi - check if it's a known non-corporate network
                $netshOutput = netsh wlan show interfaces 2>$null
                if ($netshOutput -and ($netshOutput -match 'SSID\s+:\s+(.+)')) {
                    if ($matches[1]) {
			$ssid = $matches[1].Trim()
		    } else {
			$ssid = $null
                    ...
                    # Known non-corporate SSIDs (public networks)
                    $nonCorpSSIDs = @('attwifi', 'xfinitywifi', 'starbucks', 'google starbucks', 'snafu')
                    
                    foreach ($nonCorp in $nonCorpSSIDs) {
                        if ($ssid -like "*$nonCorp*") {
                            Write-Log "DC ping succeeded but on non-corporate WiFi ($ssid) - likely stale VPN routes" "WARN"
                            return $false
                        }
                    }
                }
            ...
            Write-Log "Could not check WiFi SSID for DC validation: $_" "DEBUG"
        }
        
        # Ping succeeded and no red flags detected
        return $true
    ...
    if (-not $ciscoPath) { 
        ...
        if (-not (Test-Path $vpn_cmd)) { 
            ...
            try {
                ...
                # No-op if $BlackholeEnabled = $false or flag doesn't exist.
                ...
                # WLANi03 FAST PATH: This SSID is always on-prem corp wireless.
                # Skip gateway preflight entirely - we know ISE is involved.
                # Go straight to enroll.cisco.com to determine posture state.
                if ($networkInfo.SSID -eq 'WLANi03') {
                    Write-Log "WLANi03 detected - on-prem fast path (skipping preflight)" "INFO"
                    Start-Sleep -Seconds 2  # minimal settle time post-DHCP
                    
                    $enrollResult = Test-Redirect -Url $ISERedirectTestURL
                    if ($enrollResult.IsRedirect) {
                        $enrollType = Get-RedirectType -RedirectUrl $enrollResult.RedirectUrl
                        Write-Log "WLANi03 enroll redirect: type=$enrollType url=$($enrollResult.RedirectUrl)" "INFO"
                        
                        if ($enrollType -eq 'ISE_EMPLOYEE') {
                            Write-Log "WLANi03 unpostured - triggering rescan" "INFO"
                            ...
                            return
                        } elseif ($enrollType -eq 'ISE_GUEST') {
                            Invoke-CaptivePortalRemediation -PortalType "ISE_GUEST" -RedirectUrl $enrollResult.RedirectUrl
                            $script:exitReason = "ise_guest_browser_remediated"
                            return
                        }
                        # Unknown redirect on WLANi03 - fall through to normal flow
                        Write-Log "WLANi03 unexpected redirect type ($enrollType) - falling through to normal flow" "WARN"
                    } else {
                        # No redirect - already postured, fire on_prem and exit
                        Write-Log "WLANi03 no redirect - already postured, on-prem" "INFO"
                        Send-FlareEvent 'on_prem'
                        Clear-CaptiveEventHistory "WLANi03 on-prem postured"
                ...
                switch ($nwCheck.Status) {
                    ...
                    "unreachable" {
                        ...
                        # If we reach here: enroll didn't redirect, VPN not intermediate - genuine network problem
