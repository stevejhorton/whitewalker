...
<#
  ...
  Version: 3.14.0_RC1.20
  Author: steve.horton@optum.com (with AI assist)
  Date: 15-Jul-2026

  Major changes in 3.14.0_RC1.20:
    - NEW: CapPortalRunnerMode pivot ($CapPortalRunnerMode) - controls which cap portal runner fires
      "full" = WW_cap_portal_runner.ps1 (existing - kills Cisco browser, opens Edge, full UX)
      "base" = WW_cap_portal_runner_base.ps1 (NEW - observer mode, lets Cisco client handle it,
               escalates to full runner if user not online within $CapPortalBaseTimeoutSeconds)
    - NEW: captive_portal_browser_base flare (Event 778) - fires base runner via Task Scheduler
    - NEW: WW_cap_portal_runner_base.xml Task Scheduler trigger (Event 778 -> base runner)
    - AlwaysOn profile awareness: $vpnProfile (uhg_always_on / uhg_always_on_elevated) drives
      which mode is most appropriate - pivot is manual, not automatic, for fleet control
    - All captive_portal_browser fire sites in WW_main now route through the pivot
    - NEW: Cisco Secure Client version/module inventory wired into run header via
      WW_csc_client_info.ps1 (dot-sourced below). Cached/change-detected - zero extra
      I/O on the fast path (see WW_csc_client_info.ps1 header for details).

  Major changes in 3.14.0_RC1.19_cap1:
    - FIX: Invoke-BlackholeAction -add now synchronous (-Wait added)
      Sinkhole must be fully applied before VPN disconnect check
    - NEW: VPN disconnect after Blackhole -add (on-prem enforcement)
      After -add completes, Get-VpnState checked - if not Disconnected,
      vpncli disconnect fires and final state is logged
      Only fires inside $BlackholeEnabled=true on-prem path - no extra gate needed
      Exempt users (in allow groups) handled by Set-VpnHostsEntry.ps1 - no sinkhole
      applied = no disconnect

  Major changes in 3.14.0_RC1.19:
    - NEW: currentState / currentStateTime fields added to state.json
      Set-FlareStamp now writes current network state on every state-determining flare
      (on_prem, user_tun, mgmt_tun, off_prem_no_vpn, captive_portal, off_prem_untrusted_ssid)
      Consumed by haystack_action.ps1 and Invoke-SelfUpdate for network-aware decisions

    - NEW: Invoke-SelfUpdate - zero-touch CPR self-update from NETLOGON share
      Touch C:\Windows\Temp\cpr_update_dev.flag  -> pulls bin/DEV/cpr_current.zip
      Touch C:\Windows\Temp\cpr_update_prod.flag -> pulls bin/PROD/cpr_current.zip
      No admin required to create flag (C:\Windows\Temp writable by all users)
      DEV wins if both flags present
      Gates on currentState (on_prem/user_tun/mgmt_tun) - share must be reachable
      10-minute cooldown prevents re-trigger loops
      On failure: flag renamed to .failed, [!!] ERROR banner in main log
      Gated by $SelfUpdateEnabled = $true/$false

    - NEW: Sync-BlackholeShutdownTask - lifecycle manager for WW_blackhole_shutdown TS task
      Task fires Set-VpnHostsEntry.ps1 -rm on system shutdown (Event ID 1074)
      Clears sinkhole entries before reboot so hosts file is never left poisoned
      Task registered when $BlackholeEnabled=true, unregistered when $BlackholeEnabled=false
      Single toggle in WW_main.ps1 controls both runtime behavior AND task lifecycle
      XML: triggers/WW_blackhole_shutdown.xml (SYSTEM context, Priority=0, PT5M limit)

    - NEW: HayStack syslog offline cache (haystack_action.ps1)
      NEEDLE_HITs cached to haystack_syslog_queue.jsonl when syslog unreachable
      Queue drained FIFO on next live send opportunity
      Max 500 entries / 7 days retention - oldest dropped silently
      Network state read from state.json currentState field
      All queue logic self-contained in haystack_action.ps1 / WW_syslog.ps1

  Major changes in 3.14.0_RC1.18-cap1:
    - NEW: Trusted SSID tiebreaker for on-prem detection (closes off-prem VPN + no-ISP-suffix false positive)
      Tier 1: $HardcodedTrustedSsids array (zero I/O, instant, covers WLANi03 fast path)
      Tier 2: trusted_ssids.cfg synced from NETLOGON (cooldown gated, version+hash checked)
      Wired (Ethernet) connections bypass SSID check entirely
      on_prem flare now requires: DC=true + suffixes clean + SSID trusted
      SSID not trusted: fires off_prem_no_vpn, does NOT disconnect VPN
    - NEW: Sync-TrustedSsidFile function (mirrors Set-VpnHostsEntry.ps1 Sync-ConfigFile pattern)
    - NEW: Test-SsidTrusted function (two-tier check, wired bypass, feature gate)
  ...
  Major changes in 3.14.0_RC1.17:
    ...
    - Added logo and hotlinks directly to toast and popup notifications (still need to ppulate contact info)
  ...
  Major changes in 3.13.1_ER3 (Engineering Release 3):
    - CRITICAL FIX: Captive portal was triggering on the string "login" being in the HTML response, which was giving false positives on almost any home router. Removed that string from the array [...]
...
# Cisco Secure Client version/module inventory helper - dot-sourced early so
# Get-CiscoSecureClientInfo is available by the time Write-RunHeader runs.
# Self-contained: cached + change-detected, defensive if Write-Log isn't
# defined yet (see WW_csc_client_info.ps1 header for details).
...
# Captive Portal Runner Mode
# Controls which cap portal runner script fires when a captive portal is detected.
# "full"   = WW_cap_portal_runner.ps1 (existing - kills Cisco browser, opens Edge, full UX)
# "base"   = WW_cap_portal_runner_base.ps1 (NEW - observer mode, lets Cisco Secure Client
#             handle captive portal natively; only intervenes if user not online within timeout)
# Pivot:   flip $CapPortalRunnerMode to swap behavior fleet-wide with zero other changes.
# AlwaysOn profiles (uhg_always_on.xml / uhg_always_on_elevated.xml) benefit most from "base".
# $vpnProfile is read from HKLM:\SYSTEM\UHG\DSM at run header time.
$CapPortalRunnerMode            = "base"   # "full" | "base"
$CapPortalBaseScriptPath        = "C:\ProgramData\WhiteWalker\WW_cap_portal_runner_base.ps1"
$CapPortalFullScriptPath        = "C:\ProgramData\WhiteWalker\WW_cap_portal_runner.ps1"
$CapPortalBaseTimeoutSeconds    = 120      # base runner: seconds to observe before intervening
$CapPortalBaseCheckIntervalSec  = 10       # base runner: connectivity poll interval while observing
...
$BlackholeEnabled       = $true  # set to $true to enable VPN blackhole on on_prem flare
...
$HaystackEnabled               = $true  # set to $true to enable HayStack event monitoring
...
# XmlVault subsystem (offline VPN XML backup and island self-heal)
# Syncs VPN profile XMLs from NETLOGON to local vault; restores from vault on island detection.
# Task lifecycle managed by Sync-XmlVaultTask - mirrors BlackholeShutdown pattern.
$XmlVaultEnabled     = $true   # set to $true to enable XmlVault sync and restore
$XmlVaultScriptPath  = "C:\ProgramData\WhiteWalker\WW_xml_vault.ps1"
$XmlVaultTaskName    = "WW_xml_vault"
$XmlVaultTaskXml     = "C:\ProgramData\WhiteWalker\triggers\WW_xml_vault.xml"

# Self-Update (pull cpr_current.zip from NETLOGON and run install.ps1)
# Flag files at C:\Windows\Temp\ are writable by all standard users - no admin required.
# DEV flag -> bin/DEV/cpr_current.zip | PROD flag -> bin/PROD/cpr_current.zip
$SelfUpdateEnabled           = $true
$SelfUpdateDevFlag           = "C:\Windows\Temp\cpr_update_dev.flag"
$SelfUpdateProdFlag          = "C:\Windows\Temp\cpr_update_prod.flag"
$SelfUpdateCooldownFlag      = "C:\ProgramData\WhiteWalker\cpr_update_ran.flag"
$SelfUpdateCooldownMinutes   = 10   # prevent re-trigger loop
$SelfUpdateRemoteBase        = "\\ms.ds.uhc.com\netlogon\UHG\Scripts\AOVPN\CPR\bin"
$SelfUpdateZipName           = "cpr_current.zip"
$SelfUpdateInstallScript     = "install.ps1"

# Blackhole Shutdown Task (register/unregister WW_blackhole_shutdown TS task)
# Task fires Set-VpnHostsEntry.ps1 -rm on system shutdown (Event ID 1074)
# Lifecycle managed by WW_main on each run - mirrors BlackholeEnabled state.
$BlackholeShutdownTaskName   = "WW_blackhole_shutdown"
$BlackholeShutdownTaskXml    = "C:\ProgramData\WhiteWalker\triggers\WW_blackhole_shutdown.xml"
...
$TestDC_DiagnosticMode  = $true   # $true = full verbose suffix classification log; $false = single compact summary line

# Trusted SSID list - on-prem tiebreaker for WiFi connections
# Tier 1: hardcoded fast path - checked first, zero I/O, covers ~99% of fleet
# Tier 2: trusted_ssids.cfg from NETLOGON - checked only when Tier 1 misses
# Wired (Ethernet) connections bypass SSID check entirely.
$HardcodedTrustedSsids       = @('WLANi03')   # fast path - no file I/O
$TrustedSsidEnabled          = $true           # set $false to bypass SSID check entirely
$TrustedSsidSyncEnabled      = $true           # set $false to disable NETLOGON sync
$TrustedSsidSyncCooldownHours = 4              # min hours between NETLOGON syncs
$TrustedSsidFile             = "C:\ProgramData\WhiteWalker\trusted_ssids.cfg"
$TrustedSsidSyncFlag         = "C:\ProgramData\WhiteWalker\trusted_ssids_sync.flag"
$RemoteTrustedSsidFile       = "\\ms.ds.uhc.com\netlogon\UHG\Scripts\AOVPN\CPR\trusted_ssids.cfg"

# --------------------- Utility: Reliable Append Logging -----------------------
# =============================================================================
# Trusted SSID Helpers
# =============================================================================
function Test-ShareReachableFast {
    <#
    .SYNOPSIS
    Fast, hard-timeout TCP connect check to SMB port 445 on the target host.
    Avoids slow UNC Test-Path negotiation (can hang 10-20s+ when off-network,
    e.g. home WiFi with no route to the corp share) and avoids ICMP-block
    false negatives from ping/Test-Connection (many DCs silently drop ICMP,
    which would make Test-Connection time out the FULL timeout anyway - a
    false negative indistinguishable from "actually unreachable").

    Returns $true only if a TCP handshake to port 445 completes within
    $TimeoutMs. Returns $false on any failure, timeout, or exception -
    never throws. Worst-case blocking time is bounded by $TimeoutMs.
    #>
    param(
        [string]$HostName,
        [int]$TimeoutMs = 300
    ...
    try {
        ...
        $connectTask = $tcp.BeginConnect($HostName, 445, $null, $null)
...
function Sync-TrustedSsidFile {
    <#
    .SYNOPSIS
    Syncs trusted_ssids.cfg from NETLOGON share using version+hash check.
    Respects $TrustedSsidSyncCooldownHours to prevent thrashing.
    Falls back to local copy silently if share is unreachable.
    Only called when current SSID is NOT in $HardcodedTrustedSsids.
    #>

    if (-not $TrustedSsidSyncEnabled) {
        Write-Log "TrustedSsid: sync disabled - using local cache only" "DEBUG"
        return
    }

    # Cooldown check - don't hammer the share on every CPR run
    if (Test-Path $TrustedSsidSyncFlag) {
        try {
            $lastSync = [datetime]::Parse((Get-Content $TrustedSsidSyncFlag -Raw).Trim())
            $ageHours = ((Get-Date) - $lastSync).TotalHours
            if ($ageHours -lt $TrustedSsidSyncCooldownHours) {
                Write-Log "TrustedSsid: cache fresh ($([math]::Round($ageHours,1))h old) - skipping sync" "DEBUG"
                return
            }
            Write-Log "TrustedSsid: cache stale ($([math]::Round($ageHours,1))h old) - syncing from share" "INFO"
        } catch {
            Write-Log "TrustedSsid: could not read sync flag - forcing sync" "WARN"
        }
    } else {
        Write-Log "TrustedSsid: no sync flag - first run, syncing from share" "INFO"
    }

    # Fast bounded reachability check BEFORE any UNC Test-Path/Copy-Item.
    # Prevents multi-second hangs when off-VPN/off-prem (e.g. home WiFi) -
    # UNC negotiation against an unreachable host can block far longer than
    # this 300ms TCP probe. Fails closed: if unreachable, Test-SsidTrusted
    # falls back to the local cached trusted_ssids.cfg (if any), or returns
    # "not trusted" if no local cache exists - both are safe outcomes since
    # being unable to reach the corp share means we're almost certainly not
    # on-prem anyway.
    $shareHost = ([uri]("file:" + ($RemoteTrustedSsidFile -replace '\\','/'))).Host
    if (-not (Test-ShareReachableFast -HostName $shareHost -TimeoutMs 300)) {
        Write-Log "TrustedSsid: fast reachability check failed ($shareHost:445 unreachable in 300ms) - using local cache" "DEBUG"
        return
    }

    # Share reachability check (now only reached if fast check passed)
    if (-not (Test-Path -LiteralPath (Split-Path $RemoteTrustedSsidFile -Parent) -ErrorAction SilentlyContinue)) {
        Write-Log "TrustedSsid: share unreachable - using local cache" "WARN"
        return
    }

    if (-not (Test-Path -LiteralPath $RemoteTrustedSsidFile)) {
        Write-Log "TrustedSsid: remote file not found at $RemoteTrustedSsidFile - using local cache" "WARN"
        return
    }

    # Version check (fast path)
    try {
        $remoteVer = (Get-Content $RemoteTrustedSsidFile -TotalCount 1 -ErrorAction Stop).Trim()
        $localVer  = if (Test-Path $TrustedSsidFile) { (Get-Content $TrustedSsidFile -TotalCount 1 -ErrorAction SilentlyContinue).Trim() } else { "" }
        if ($remoteVer -and $localVer -and ($remoteVer -eq $localVer)) {
            Write-Log "TrustedSsid: version match ($remoteVer) - no pull needed" "DEBUG"
            (Get-Date -Format "o") | Set-Content $TrustedSsidSyncFlag -Encoding UTF8 -Force
            return
        }
    } catch { }
    ...
    # Pull from share
    try {
        Copy-Item -LiteralPath $RemoteTrustedSsidFile -Destination $TrustedSsidFile -Force -ErrorAction Stop
        (Get-Date -Format "o") | Set-Content $TrustedSsidSyncFlag -Encoding UTF8 -Force
        Write-Log "TrustedSsid: updated from share" "INFO"
    } catch {
        Write-Log "TrustedSsid: failed to pull from share: $_ - using local cache" "WARN"
    }
}

function Test-SsidTrusted {
    <#
    .SYNOPSIS
    Returns $true if the current SSID is trusted (on-prem confirmed).
    Wired (Ethernet) connections always return $true - SSID is irrelevant.
    Checks Tier 1 (hardcoded array) first - zero I/O, instant.
    Falls through to Tier 2 (trusted_ssids.cfg) only on a miss.
    Returns $true if check is disabled via $TrustedSsidEnabled = $false.
    #>
    param(
        [object]$NetworkInfo
    )
    ...
    # Feature gate
    if (-not $TrustedSsidEnabled) {
        Write-Log "TrustedSsid: check disabled - skipping" "DEBUG"
        return $true
    }

    # Wired - SSID is meaningless, skip check entirely
    if ($NetworkInfo.ConnectionType -eq "Ethernet") {
        Write-Log "TrustedSsid: wired connection - SSID check skipped" "DEBUG"
        return $true
    }

    $currentSsid = $NetworkInfo.SSID

    if ([string]::IsNullOrWhiteSpace($currentSsid) -or $currentSsid -eq "N/A") {
        Write-Log "TrustedSsid: no SSID detected - cannot confirm trusted" "WARN"
        return $false
    }

    # Tier 1: hardcoded fast path - zero I/O
    if ($HardcodedTrustedSsids -contains $currentSsid) {
        Write-Log "TrustedSsid: '$currentSsid' matched hardcoded list - trusted (fast path)" "DEBUG"
        return $true
    }

    Write-Log "TrustedSsid: '$currentSsid' not in hardcoded list - checking trusted_ssids.cfg" "DEBUG"

    # Tier 2: sync + check cfg file
    Sync-TrustedSsidFile

    if (-not (Test-Path $TrustedSsidFile)) {
        Write-Log "TrustedSsid: no local trusted_ssids.cfg found - SSID not trusted" "WARN"
        return $false
    }

    try {
        $trustedSsids = Get-Content $TrustedSsidFile -ErrorAction Stop |
            Where-Object { $_ -notmatch '^\s*#' -and $_ -notmatch '^\s*version:' -and $_.Trim() -ne '' } |
            ForEach-Object { $_.Trim() }

        if ($trustedSsids -contains $currentSsid) {
            Write-Log "TrustedSsid: '$currentSsid' matched trusted_ssids.cfg - trusted" "INFO"
            return $true
        } else {
            Write-Log "TrustedSsid: '$currentSsid' NOT in trusted_ssids.cfg - SSID not trusted" "WARN"
            return $false
        }
    } catch {
        Write-Log "TrustedSsid: error reading trusted_ssids.cfg: $_ - SSID not trusted" "WARN"
        return $false
    }
}
...
function Write-RunHeader {
  ...
  # Cisco Secure Client version + module inventory (cached, change-detected -
  # zero manifest I/O on the fast path; see WW_csc_client_info.ps1 header)
  try {
    $cscInfo = Get-CiscoSecureClientInfo
    Add-LogLine ("csc_version={0}  source={1}" -f $cscInfo.CoreVersion, $cscInfo.Source)
    if ($global:CscInfoLogModulesAlways -or $WWDebug -or $WWTrace) {
      $moduleSummary = ($cscInfo.Modules | ForEach-Object { "$($_.Module)=$($_.Version)" }) -join ', '
      Add-LogLine ("csc_modules={0}" -f $moduleSummary)
    }
  } catch {
    Add-LogLine ("csc_version=Unknown  source=error: {0}" -f $_)
  }
  ...
  Add-LogLine ("xml_vault_enabled={0}  script={1}" -f $XmlVaultEnabled, $XmlVaultScriptPath)
  Add-LogLine ("cap_portal_runner_mode={0}  base_script={1}" -f $CapPortalRunnerMode, $CapPortalBaseScriptPath)
...
# Network state flares - these update currentState in state.json.
# Action flares (e.g. captive_portal_browser) do NOT update currentState.
$script:StateFlares = @('on_prem','user_tun','mgmt_tun','off_prem_no_vpn','captive_portal','off_prem_untrusted_ssid')
...
function Set-FlareStamp([string]$key) {
  ...
  # Update currentState for state-determining flares only
  if ($script:StateFlares -contains $key) {
    if (-not ($global:_state.PSObject.Properties.Name -contains 'currentState')) {
      $global:_state | Add-Member -NotePropertyName currentState -NotePropertyValue '' -Force
      $global:_state | Add-Member -NotePropertyName currentStateTime -NotePropertyValue '' -Force
    }
    $global:_state.currentState     = $key
    $global:_state.currentStateTime = (Get-Date).ToString('o')
    Write-Log "State: currentState updated to '$key'" "DEBUG"
  }
...
function Start-CiscoBrowserKiller {
    ...
    try {
        ...
        $killerJob = Start-Job -ScriptBlock {
            ...
            $endTime = (Get-Date).AddSeconds(120)  # Run for 120 seconds - covers full WW execution window
            $killCount = 0
        ...
        Write-Log "Cisco browser killer job started (JobId: $($killerJob.Id)) - will run for 120 seconds" "DEBUG"
        return $killerJob
...
function Stop-CiscoBrowserKiller {
    <#
    .SYNOPSIS
    Stop Cisco browser killer background job immediately (fire and forget).
    -Wait on Receive-Job was blocking the main thread for the full job duration.
    We don't need the return value - just kill it and move on.
    #>
    param($Job)
...
function Sync-BlackholeShutdownTask {
    <#
    .SYNOPSIS
    Ensures WW_blackhole_shutdown TS task exists when BlackholeEnabled=true
    and removes it when BlackholeEnabled=false. Called once at startup.
    Keeps the single $BlackholeEnabled toggle in WW_main.ps1 in full control
    of a task that fires independently (on shutdown event ID 1074).
    #>
    $taskName = $BlackholeShutdownTaskName
    $taskXml  = $BlackholeShutdownTaskXml
    $exists   = $null -ne (Get-ScheduledTask -TaskName $taskName -ErrorAction SilentlyContinue)

    if ($BlackholeEnabled) {
        if ($exists) {
            Write-Log "BlackholeShutdown: task already registered - OK" "DEBUG"
            return
        }
        if (-not (Test-Path $taskXml)) {
            Write-Log "BlackholeShutdown: XML not found at $taskXml - cannot register task" "WARN"
            return
        }
        try {
            $xml = Get-Content $taskXml -Raw -ErrorAction Stop
            Register-ScheduledTask -TaskName $taskName -Xml $xml -Force -ErrorAction Stop | Out-Null
            Write-Log "BlackholeShutdown: task registered" "INFO"
        } catch {
            Write-Log "BlackholeShutdown: failed to register task: $_" "WARN"
        }
    } else {
        if (-not $exists) {
            Write-Log "BlackholeShutdown: task not present (disabled) - OK" "DEBUG"
            return
        }
        try {
            Unregister-ScheduledTask -TaskName $taskName -Confirm:$false -ErrorAction Stop
            Write-Log "BlackholeShutdown: task unregistered (BlackholeEnabled=false)" "INFO"
        } catch {
            Write-Log "BlackholeShutdown: failed to unregister task: $_" "WARN"
        }
    }
}

function Sync-XmlVaultTask {
    <#
    .SYNOPSIS
    Ensures WW_xml_vault TS task exists when XmlVaultEnabled=true
    and removes it when XmlVaultEnabled=false. Called once at startup.
    Keeps the single $XmlVaultEnabled toggle in WW_main.ps1 in full control
    of a task that fires independently (on event IDs 801/802).
    #>
    $taskName = $XmlVaultTaskName
    $taskXml  = $XmlVaultTaskXml
    $exists   = $null -ne (Get-ScheduledTask -TaskName $taskName -ErrorAction SilentlyContinue)

    if ($XmlVaultEnabled) {
        if ($exists) {
            Write-Log "XmlVault: task already registered - OK" "DEBUG"
            return
        }
        if (-not (Test-Path $taskXml)) {
            Write-Log "XmlVault: XML not found at $taskXml - cannot register task" "WARN"
            return
        }
        try {
            $xml = Get-Content $taskXml -Raw -ErrorAction Stop
            Register-ScheduledTask -TaskName $taskName -Xml $xml -Force -ErrorAction Stop | Out-Null
            Write-Log "XmlVault: task registered" "INFO"
        } catch {
            Write-Log "XmlVault: failed to register task: $_" "WARN"
        }
    } else {
        if (-not $exists) {
            Write-Log "XmlVault: task not present (disabled) - OK" "DEBUG"
            return
        }
        try {
            Unregister-ScheduledTask -TaskName $taskName -Confirm:$false -ErrorAction Stop
            Write-Log "XmlVault: task unregistered (XmlVaultEnabled=false)" "INFO"
        } catch {
            Write-Log "XmlVault: failed to unregister task: $_" "WARN"
        }
    }
}
...
function Invoke-BlackholeAction {
    ...
    if ($onPremTags -contains $Tag) {
        ...
        Write-Log "Blackhole: on-prem state ($Tag) - firing -add (synchronous)" "INFO"
        if (-not $WhatIf) {
            # Synchronous so sinkhole is applied before VPN disconnect check below
            Start-Process "powershell.exe" `
            ...
            Write-Log "Blackhole: -add complete, flag created" "INFO"

            # VPN disconnect - only fires here ($BlackholeEnabled=true, on-prem confirmed).
            # Sinkhole is now applied. If VPN is still active, take it down cleanly.
            $postBhVpnState = Get-VpnState
            if ($postBhVpnState -ne 'Disconnected') {
                Write-Log "Blackhole -add complete - VPN still active ($postBhVpnState) - disconnecting" "INFO"
                try {
                    & $vpn_cmd disconnect | Out-Null
                    Start-Sleep -Seconds 2
                    $finalVpnState = Get-VpnState
                    Write-Log "VPN disconnected - on-prem sinkhole enforced (final state: $finalVpnState)" "INFO"
                } catch {
                    Write-Log "Error disconnecting VPN after blackhole -add: $_" "WARN"
                }
            } else {
                Write-Log "Blackhole -add complete - VPN already Disconnected" "DEBUG"
            }
        } else {
            Write-Log "WHATIF: Would run blackhole.ps1 -add, create flag, and disconnect VPN if active" "INFO"
        }
    } else {
...
function Send-FlareEvent {
  ...
  # Cooldown check - BUT BYPASS for critical captive portal runner events
  # This ensures the runner ALWAYS launches when captive portal is detected
  $bypassCooldown = ($Tag -eq "captive_portal_browser" -or $Tag -eq "captive_portal_browser_base")
    ...
    try {
        ...
        if ($activeAdapter) {
            ...
            if ($ipConfig) {
                ...
                # DNS Search Suffixes (DHCP-pushed search list + computer domain membership)
                try {
                    $suffixList = [System.Collections.Generic.List[string]]::new()
                    # Method 1: Global DNS suffix search list (DHCP/GPO-pushed)
                    $dnsGlobal = Get-DnsClientGlobalSetting -ErrorAction SilentlyContinue
                    if ($dnsGlobal -and $dnsGlobal.SuffixSearchList) {
                        foreach ($s in $dnsGlobal.SuffixSearchList) {
                            $clean = $s.ToLower().Trim()
                            if (-not [string]::IsNullOrWhiteSpace($clean)) { $suffixList.Add($clean) | Out-Null }
                        }
                    }
                    # Method 2: Primary DNS suffix from computer domain membership
                    try {
                        ...
                        if (-not [string]::IsNullOrWhiteSpace($primary)) {
                            ...
                            if ($cleanPrimary -notin @('(none)', 'none') -and $suffixList -notcontains $cleanPrimary) {
                                $suffixList.Add($cleanPrimary) | Out-Null
                            }
                        }
                    } catch { }
                    $netInfo.DNSSuffixes = $suffixList | Select-Object -Unique
                } catch {
                    Write-Log "Could not retrieve DNS suffixes: $_" "DEBUG"
                }
            }
        }
    } catch {
...
function Write-NetworkInfoToLog {
    ...
    if ($NetworkInfo.DNSSuffixes -and $NetworkInfo.DNSSuffixes.Count -gt 0) {
        Write-Log "  DNS Suffixes: $($NetworkInfo.DNSSuffixes -join ', ')" "INFO"
    } else {
        Write-Log "  DNS Suffixes: (none discovered)" "DEBUG"
    }
    Write-Log "===========================" "INFO"
    ...
    if ($NetworkInfo.ConnectionType -eq "WiFi") {
        $macRand = Get-MacRandomizationState -MacAddress $NetworkInfo.MACAddress
...
function Get-MacRandomizationState {
    <#
    .SYNOPSIS
    Detect MAC randomization by inspecting the second hex digit of the MAC address.

    .DESCRIPTION
    The second hex character of a MAC encodes bits 1-0 of the first octet:
      Bit 1 = locally administered (set by OS when randomizing)
      Bit 0 = multicast (always 0 for unicast NICs)

    Second char in {2,6,A,E} => locally administered unicast => randomized.
    No registry traversal required - the MAC itself is the ground truth.

    Returns a hashtable with:
      ModeValue         - 1 if randomized, 0 if not, -1 if MAC unavailable
      RandomizationMode - "randomized" | "disabled" | "unknown"
      IsRandomized      - $true if any randomization detected
      IsRisky           - $true if randomized (mode unknown - treat as risky)
    #>
    param(
        [string]$MacAddress = ""
    )
    ...
    try {
        if ([string]::IsNullOrEmpty($MacAddress)) {
            Write-Log "MacRandState: no MAC address provided" "DEBUG"
            return $result
        }

        $mac        = $MacAddress.ToUpper().Trim()
        $secondChar = $mac[1].ToString()
        $isRand     = $secondChar -in @('2','6','A','E')

        $result.ModeValue         = if ($isRand) { 1 } else { 0 }
        $result.IsRandomized      = $isRand
        $result.IsRisky           = $isRand   # specific mode unknown - any randomization is portal-risky
        $result.RandomizationMode = if ($isRand) { "randomized" } else { "disabled" }

        Write-Log "MacRandState: MAC=$mac second-char='$secondChar' randomized=$isRand" "DEBUG"

    } catch {
        Write-Log "MacRandState: error: $_" "DEBUG"
    }
...
function Test-DC { 
    ...
    try {
        if ($TestDC_DiagnosticMode) { Write-Log "=== TEST-DC START (Diagnostic Mode) ===" "INFO" }
        if ($TestDC_DiagnosticMode) { Write-Log "Testing DC hostname: $hostname" "DEBUG" }

        $pingResult = Test-Connection -ComputerName $hostname -Count 1 -Quiet -ErrorAction Stop
        if ($TestDC_DiagnosticMode) { Write-Log "DC Ping result: $pingResult" "DEBUG" }
        if (-not $pingResult) {
            if ($TestDC_DiagnosticMode) {
                Write-Log "DC not reachable - returning off-prem" "DEBUG"
                Write-Log "=== TEST-DC END (result=FALSE) ===" "INFO"
            } else {
                Write-Log "Test-DC: host=$hostname  ping=False  corp_suffixes=(none)  non_corp=(none)  result=False" "INFO"
            }
            return $false
        }
        ...
        try {
            ...
            if ($TestDC_DiagnosticMode) { Write-Log "[SUFFIX-COLLECTION] Starting Method 1: Get-DnsClientGlobalSetting" "DEBUG" }
            try {
                $dnsGlobal = Get-DnsClientGlobalSetting -ErrorAction Stop
                if ($dnsGlobal.SuffixSearchList) {
                    if ($TestDC_DiagnosticMode) { Write-Log "[SUFFIX-COLLECTION] Method 1 returned list:" "DEBUG" }
                    foreach ($suffix in $dnsGlobal.SuffixSearchList) {
                        $cleanSuffix = $suffix.ToLower().Trim()
                        if ($TestDC_DiagnosticMode) { Write-Log "  [SUFFIX-METHOD1] Found: '$cleanSuffix'" "DEBUG" }
                        $foundSuffixes += $cleanSuffix
                    }
                } else {
                    if ($TestDC_DiagnosticMode) { Write-Log "[SUFFIX-COLLECTION] Method 1: SuffixSearchList is empty/null" "DEBUG" }
                }
            } catch {
                if ($TestDC_DiagnosticMode) { Write-Log "[SUFFIX-COLLECTION] Method 1 FAILED: $_" "DEBUG" }
            }

            # Method 2: Get primary DNS suffix (computer's domain membership)
            if ($TestDC_DiagnosticMode) { Write-Log "[SUFFIX-COLLECTION] Starting Method 2: IPGlobalProperties.DomainName" "DEBUG" }
            try {
                $primary = [System.Net.NetworkInformation.IPGlobalProperties]::GetIPGlobalProperties().DomainName
                if (-not [string]::IsNullOrWhiteSpace($primary)) {
                    $cleanPrimary = $primary.ToLower().Trim()
                    if ($cleanPrimary -in @('(none)', 'none')) {
                        if ($TestDC_DiagnosticMode) { Write-Log "[SUFFIX-COLLECTION] Method 2 ignored placeholder DomainName: '$cleanPrimary'" "DEBUG" }
                    } else {
                        if ($TestDC_DiagnosticMode) { Write-Log "  [SUFFIX-METHOD2] Found: '$cleanPrimary'" "DEBUG" }
                        $foundSuffixes += $cleanPrimary
                    }
                } else {
                    if ($TestDC_DiagnosticMode) { Write-Log "[SUFFIX-COLLECTION] Method 2: DomainName is empty/null" "DEBUG" }
                }
            } catch {
                if ($TestDC_DiagnosticMode) { Write-Log "[SUFFIX-COLLECTION] Method 2 FAILED: $_" "DEBUG" }
            }
            ...
            if (-not $foundSuffixes) {
                Write-Log "[WARN] RESULT: DC ping succeeded but NO DNS suffixes found - cannot confirm on-prem" "WARN"
                if ($TestDC_DiagnosticMode) {
                    Write-Log "--- RETURNING: FALSE (off-prem assumed) ---" "INFO"
                    Write-Log "=== TEST-DC END (result=FALSE, no suffixes) ===" "INFO"
                } else {
                    Write-Log "Test-DC: host=$hostname  ping=$pingResult  corp_suffixes=(none)  non_corp=(none)  result=False" "INFO"
                }
                return $false
            }
            ...
            foreach ($found in $foundSuffixes) {
                ...
                foreach ($corp in $corpSuffixes) {
                    ...
                    if ($found -eq $corp -or $found -like "*.$corp") {
                        $isCorp = $true
                        $corpSuffixCount++
                        $corpSuffixList += $found
                        if ($TestDC_DiagnosticMode) { Write-Log "  [OK] CORPORATE: '$found' matches pattern '$corp'" "INFO" }
                        break
                    }
                }
                if (-not $isCorp) {
                    $nonCorpSuffixes += $found
                    if ($TestDC_DiagnosticMode) { Write-Log "  [!!] NON-CORPORATE: '$found' (does NOT match any trusted pattern)" "INFO" }
                }
            }
            ...
            if ($corpSuffixCount -eq 0) {
                ...
                if ($TestDC_DiagnosticMode) {
                    ...
                    Write-Log "Test-DC: host=$hostname  ping=$pingResult  corp_suffixes=(none)  non_corp=$($nonCorpSuffixes -join ',')  result=False" "INFO"
                }
                return $false
            }
            ...
            if ($nonCorpSuffixes.Count -gt 0) {
                ...
                if ($TestDC_DiagnosticMode) {
                    ...
                    $corpStr = if ($corpSuffixList) { $corpSuffixList -join ',' } else { '(none)' }
                    Write-Log "Test-DC: host=$hostname  ping=$pingResult  corp_suffixes=$corpStr  non_corp=$($nonCorpSuffixes -join ',')  result=False" "INFO"
                }
                return $false
            }
            ...
            if ($TestDC_DiagnosticMode) {
                ...
                Write-Log "Test-DC: host=$hostname  ping=$pingResult  corp_suffixes=$($foundSuffixes -join ',')  non_corp=0  result=True" "INFO"
            }
            return $true
            
        } catch {
            # Suffix check failed - fall through and trust the ping
            Write-Log "[WARN] Could not validate DNS suffixes: $_" "WARN"
            ...
            if ($TestDC_DiagnosticMode) {
                ...
                Write-Log "Test-DC: host=$hostname  ping=$pingResult  corp_suffixes=(error)  non_corp=(error)  result=True (fallback)" "INFO"
            }
            return $true
        }
        ...
        if ($TestDC_DiagnosticMode) {
            ...
            Write-Log "Test-DC: host=$hostname  result=False (outer exception)" "INFO"
        }
        return $false 
    }
}
...
function Invoke-CaptivePortalRemediation {
    ...
    try {
        ...
        if (-not $WhatIf) {
            ...
            # Start Cisco browser killer NOW - only fires in captive portal path
            $script:ciscoBrowserKillerJob = Start-CiscoBrowserKiller
            ...
            # Cap portal runner pivot - route to base observer or full runner based on $CapPortalRunnerMode
            if ($CapPortalRunnerMode -eq "base") {
                Write-Log "CapPortalRunner: mode=base - firing captive_portal_browser_base (Event 778)" "INFO"
                Send-FlareEvent 'captive_portal_browser_base'
            } else {
                Write-Log "CapPortalRunner: mode=full - firing captive_portal_browser (Event 777)" "INFO"
...
function Test-NwCheckAfterCompliance {
    ...
    Write-Log "Verifying nwcheck accessibility after ISE compliance (up to $TimeoutSec seconds)..." "INFO"
...
function Test-OnPremBySuffix {
    <#
    .SYNOPSIS
    Lightweight on-prem detection via DNS suffix inspection only - no network required.
    Returns $true if ONLY corporate suffixes present (on-prem).
    Returns $false if any non-corp suffix present (off-prem or VPN from home).
    Reuses same corp suffix list as Test-DC. Called by Invoke-SsidChangeCheck
    to determine whether a Connected VPN should be kicked on network change.
    #>
    $corpSuffixes = @('ms.ds.uhc.com', 'ds.uhc.com', 'uhc.com')
    try {
        $foundSuffixes = @()
        ...
        try {
            ...
            if (-not [string]::IsNullOrWhiteSpace($primary)) {
                $cleanPrimary = $primary.ToLower().Trim()
                if ($cleanPrimary -notin @('(none)', 'none')) { $foundSuffixes += $cleanPrimary }
            }
        } catch { }

        $foundSuffixes = $foundSuffixes | Where-Object { -not [string]::IsNullOrWhiteSpace($_) } | Select-Object -Unique
        if (-not $foundSuffixes) { return $false }  # no suffixes - can't confirm on-prem

        foreach ($found in $foundSuffixes) {
            $isCorp = $false
            foreach ($corp in $corpSuffixes) {
                if ($found -eq $corp -or $found -like "*.$corp") { $isCorp = $true; break }
            }
            if (-not $isCorp) {
                Write-Log "OnPremSuffix: non-corp suffix '$found' present - off-prem" "DEBUG"
                return $false
            }
        }
        Write-Log "OnPremSuffix: only corp suffixes present - on-prem confirmed" "DEBUG"
        return $true
    } catch {
        Write-Log "OnPremSuffix: check failed: $_" "DEBUG"
        return $false  # safe default - don't disconnect VPN if unsure
    }
}
...
function Invoke-SsidChangeCheck {
    ...
    # Transitional blocking states (Reconnecting/Connecting) - VPN is blocking
    # 802.1x and ISE posture on the new network. Disconnect immediately regardless
    # of SSID - AnyConnect has no awareness of time passed since original disconnect.
    if ($vpnState -in @('Reconnecting', 'Connecting')) {
        Write-Log "VPN in blocking state '$vpnState' on SSID change - disconnecting immediately" "WARN"
        try {
            & $vpn_cmd disconnect | Out-Null
            Start-Sleep -Seconds 2
            Write-Log "VPN cleared from blocking state" "INFO"
        } catch { Write-Log "Error disconnecting VPN in blocking state: $_" "WARN" }
        return
    }

    # VPN is Connected - check if we're actually on-prem via DNS suffixes (no network needed).
    # On-prem = only corp suffixes present. Off-prem = foreign suffixes in the list.
    # This works regardless of SSID name - Ed can rename WLANi03 and it still works.
    if ($vpnState -eq 'Connected') {
        if (Test-OnPremBySuffix) {
            Write-Log "VPN Connected but on-prem confirmed by DNS suffixes - disconnecting" "WARN"
            try {
                & $vpn_cmd disconnect | Out-Null
                Start-Sleep -Seconds 2
                Write-Log "VPN disconnected - on-prem confirmed" "INFO"
            } catch { Write-Log "Error disconnecting VPN on on-prem detection: $_" "WARN" }
            Send-WwNotification -Title "VPN Disabled On This Network" `
                -Body "You are connected directly to the corporate network. VPN is not required here and has been disabled to improve performance and battery life." `
                -Duration 10
        } else {
            Write-Log "VPN Connected, off-prem confirmed by DNS suffixes - leaving VPN alone" "INFO"
        }
        return
    }

    # Remaining transitional states between off-prem networks: grace window
    ...
    while ($elapsed -lt $VpnSsidChangeGraceSec) {
        ...
        if ($vpnState -eq "Disconnected") {
            Write-Log "VPN disconnected within grace window" "INFO"
            $resolved = $true; break
        }
    }
...
function Invoke-SelfUpdate {
    <#
    .SYNOPSIS
    Checks for cpr_update_dev.flag or cpr_update_prod.flag in C:\Windows\Temp.
    If found and network state allows, pulls cpr_current.zip from NETLOGON
    bin/DEV or bin/PROD channel, extracts to $env:TEMP, and runs install.ps1.
    Renames flag to .failed on error so techs can see it. Never touches logs.
    Gated by $SelfUpdateEnabled.
    #>

    # Determine which flag (if any) is present. DEV wins if both present.
    $channel  = $null
    $flagFile = $null
    if (Test-Path $SelfUpdateDevFlag)  { $channel = "DEV";  $flagFile = $SelfUpdateDevFlag }
    elseif (Test-Path $SelfUpdateProdFlag) { $channel = "PROD"; $flagFile = $SelfUpdateProdFlag }

    if (-not $channel) { return }  # no flag, nothing to do

    if (-not $SelfUpdateEnabled) {
        Write-Log "SelfUpdate: $channel flag found at $flagFile but SelfUpdateEnabled=false - skipping (flag left in place)" "WARN"
        return
    }

    Write-Log "SelfUpdate: $channel flag detected - checking eligibility" "INFO"

    # Cooldown check - prevent re-trigger loop
    if (Test-Path $SelfUpdateCooldownFlag) {
        try {
            $lastRan  = [datetime]::Parse((Get-Content $SelfUpdateCooldownFlag -Raw).Trim())
            $ageMin   = ((Get-Date) - $lastRan).TotalMinutes
            if ($ageMin -lt $SelfUpdateCooldownMinutes) {
                Write-Log "SelfUpdate: cooldown active ($([math]::Round($ageMin,1))m ago) - skipping" "WARN"
                return
            }
        } catch { }
    }

    # Network state check - only proceed if we can reach the share
    $lastState = ""
    try {
        if (Test-Path $StateFile) {
            $s = Get-Content $StateFile -Raw | ConvertFrom-Json
            $lastState = $s.currentState
        }
    } catch { }

    $goodStates = @("on_prem","user_tun","mgmt_tun")
    if ($goodStates -notcontains $lastState) {
        Write-Log "SelfUpdate: $channel flag found but network state is '$lastState' - share likely unreachable. Will retry when on-prem or VPN." "WARN"
        return
    }

    Write-Log "SelfUpdate: proceeding - channel=$channel state=$lastState" "INFO"

    $remoteZip = "\\ms.ds.uhc.com\netlogon\UHG\Scripts\AOVPN\CPR\bin\$channel\$SelfUpdateZipName"
    $localZip  = Join-Path $env:TEMP "cpr_update_$channel.zip"
    $extractDir= Join-Path $env:TEMP "cpr_update_$channel"

    try {
        # Verify share is reachable
        if (-not (Test-Path $remoteZip)) {
            throw "Remote zip not found: $remoteZip"
        }

        # Pull zip
        Write-Log "SelfUpdate: pulling $remoteZip -> $localZip" "INFO"
        Copy-Item -LiteralPath $remoteZip -Destination $localZip -Force -ErrorAction Stop

        # Clean and extract
        if (Test-Path $extractDir) { Remove-Item $extractDir -Recurse -Force -ErrorAction SilentlyContinue }
        Write-Log "SelfUpdate: extracting to $extractDir" "INFO"
        Expand-Archive -LiteralPath $localZip -DestinationPath $extractDir -Force -ErrorAction Stop

        # Find and run install.ps1
        $installScript = Get-ChildItem -Path $extractDir -Filter $SelfUpdateInstallScript -Recurse | Select-Object -First 1
        if (-not $installScript) {
            throw "install.ps1 not found in extracted zip at $extractDir"
        }

        Write-Log "SelfUpdate: running $($installScript.FullName)" "INFO"
        $proc = Start-Process "powershell.exe" `
            -ArgumentList "-ExecutionPolicy Bypass -WindowStyle Hidden -File `"$($installScript.FullName)`"" `
            -WindowStyle Hidden -Wait -PassThru -ErrorAction Stop

        if ($proc.ExitCode -ne 0) {
            throw "install.ps1 exited with code $($proc.ExitCode)"
        }

        # Success - clean up and stamp cooldown
        Remove-Item $flagFile -Force -ErrorAction SilentlyContinue
        Remove-Item $localZip -Force -ErrorAction SilentlyContinue
        (Get-Date).ToString("o") | Set-Content $SelfUpdateCooldownFlag -Encoding UTF8 -Force
        Write-Log "SelfUpdate: $channel install completed successfully - flag removed" "INFO"

    } catch {
        $failedFlag = $flagFile -replace '\.(flag)$', '.failed'
        Write-Log "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" "ERROR"
        Write-Log "[!!] SELF-UPDATE FAILED - channel=$channel" "ERROR"
        Write-Log "[!!] Error: $_" "ERROR"
        Write-Log "[!!] Flag renamed to: $failedFlag" "ERROR"
        Write-Log "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" "ERROR"
        try {
            Rename-Item -Path $flagFile -NewName (Split-Path $failedFlag -Leaf) -Force -ErrorAction Stop
        } catch {
            Write-Log "SelfUpdate: could not rename flag file: $_" "WARN"
        }
        Remove-Item $localZip -Force -ErrorAction SilentlyContinue
    }
}
...
if ($MyInvocation.InvocationName -ne '.') {
    ...
    if (-not $ciscoPath) { 
        ...
        if (-not (Test-Path $vpn_cmd)) { 
            ...
            try {
                Write-Log "CPR+ v$ver starting..."

                # Self-update check - runs before anything else so a new version
                # can replace this run entirely if needed.
                Invoke-SelfUpdate

                # Blackhole shutdown task - sync lifecycle with $BlackholeEnabled
                Sync-BlackholeShutdownTask
                ...
                # XmlVault - task lifecycle sync + startup sync trigger
                Sync-XmlVaultTask
                if ($XmlVaultEnabled) {
                    if (Test-Path $XmlVaultScriptPath) {
                        Write-Log "XmlVault: enabled - invoking startup sync" "INFO"
                        if (-not $WhatIf) {
                            Start-Process "powershell.exe" `
                                -ArgumentList "-ExecutionPolicy Bypass -WindowStyle Hidden -File `"$XmlVaultScriptPath`"" `
                                -WindowStyle Hidden
                        } else {
                            Write-Log "WHATIF: Would invoke WW_xml_vault.ps1 (sync mode)" "INFO"
                        }
                    } else {
                        Write-Log "XmlVault: enabled but script not found: $XmlVaultScriptPath" "WARN"
                    }
                } else {
                    Write-Log "XmlVault: disabled - skipping" "DEBUG"
                }
                ...
                # Cisco browser killer fires only in captive portal branch
                # (Invoke-CaptivePortalRemediation / walled garden). Init null for finally safety net.
                $ciscoBrowserKillerJob = $null
                ...
                switch ($nwCheck.Status) {
                    ...
                    "online" {
                        ...
                        if ($dcOK) {
                            # DC reachable - verify on-prem with SSID trust check.
                            # Wired = always trusted. WiFi = must be in hardcoded list or trusted_ssids.cfg.
                            # Protects against: off-prem VPN + ISP hands out no suffix
                            # (VPN pushes corp-only suffixes making DC check look on-prem).
                            $ssidTrusted = Test-SsidTrusted -NetworkInfo $networkInfo
                            if ($ssidTrusted) {
                                # FLARE EVENT: on_prem
                                # Context: SYSTEM (Event ID 793) | Reason: DC reachable, no VPN, SSID trusted
                                ...
                                # DC reachable but SSID not trusted - off-prem on VPN with no ISP suffix.
                                # Do NOT disconnect VPN. Let normal VPN flare fire on next run.
                                Write-Log "DC reachable but SSID '$($networkInfo.SSID)' not trusted - treating as off-prem, leaving VPN alone" "WARN"
                                Send-FlareEvent 'off_prem_no_vpn'
                                Clear-CaptiveEventHistory "off-premises - untrusted SSID"
                                $script:exitReason = "off_prem_untrusted_ssid"
                            }
                        } else {
                    ...
                    "unreachable" {
                        ...
                        if ($walledGarden.IsWalledGarden) {
                            ...
                            # Start Cisco browser killer NOW - captive portal path only
                            $script:ciscoBrowserKillerJob = Start-CiscoBrowserKiller
                            # Cap portal runner pivot - route to base observer or full runner based on $CapPortalRunnerMode
                            if ($CapPortalRunnerMode -eq "base") {
                                Write-Log "CapPortalRunner: mode=base - firing captive_portal_browser_base (Event 778)" "INFO"
