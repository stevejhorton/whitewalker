...
function Invoke-SyslogSend {
    ...
    if (-not (Test-Path $WWSyslogScript)) {
        Write-HALog "WW_syslog.ps1 not found at $WWSyslogScript - syslog skipped" "WARN"
        return
    }
    ...
    try {
        ...
        if ($ExtraInfo) { $args += '-ExtraInfo'; $args += $ExtraInfo }

        # Fire and forget - don't block haystack_action on syslog latency
        Start-Process powershell.exe -ArgumentList $args -WindowStyle Hidden -ErrorAction Stop | Out-Null
        Write-HALog "Syslog fired: $Payload$(if ($ExtraInfo) { " ExtraInfo=`"$ExtraInfo`"" })" "DEBUG"
    } catch {
        Write-HALog "Syslog send failed (non-fatal): $_" "WARN"
    }
}
